...
function Brick:init(x, y, playState)
...
function Brick:hit()
    ...
    -- if we're already at the lowest color, else just go down a colors
...
function Brick:update(dt)
    ...
    if self.color == 6 and self.tier == 1 and self.inPlay then
        if not gameState == 'keyObtained' then
             self.tier = 1
             self.color = 6 
        elseif gameState == 'keyObtained' then 
             self.tier = 0
        end
     end
end
...
function Brick:render()
    if self.inPlay then
        if self.locked then 
                ...
                if self.locked and self.renderUnlockedBrick == true then
